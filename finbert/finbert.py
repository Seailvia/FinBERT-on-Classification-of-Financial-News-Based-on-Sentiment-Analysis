...
class FinSentProcessor:
    def get_examples(self, data_dir, phase):
        file_path = os.path.join(data_dir, f"{phase}.csv")
...
class FinBert(object):
    ...
    def prepare_model(self, label_list):
        ...
        if self.config.local_rank == -1 or self.config.no_cuda:
            self.device = torch.device('cuda:0' if torch.cuda.is_available() and not self.config.no_cuda else "cpu")
            print(self.device)
            self.n_gpu = torch.cuda.device_count()
        else:
            torch.cuda.set_device(self.config.local_rank)
            self.device = torch.device('cuda:0', self.config.local_rank)
            self.n_gpu = 1
        ...
        if os.path.exists(self.config.model_dir) and os.listdir(self.config.model_dir):
            import shutil
            shutil.rmtree(self.config.model_dir)  # 删除整个目录
    ...
    def get_data(self, phase):
        ...
        if phase == 'train':
            train = pd.read_csv(os.path.join(self.config.data_dir, 'train1.csv'), usecols=['label', 'text'])
            print(train.columns)
            weights = list()
            labels = self.label_list
            # print(labels)
            # print(train.shape[0])
            # print(train[train['label'] == labels[0]].shape[0])
            # print(train[train['label'] == labels[1]].shape[0])
            # print(train[train['label'] == labels[2]].shape[0])
